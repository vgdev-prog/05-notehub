import type {Note, NoteCreate, NoteResponseAll, NoteUpdate} from "../types";
import {http} from "../libs/api-service.ts";
import {routes} from "../constants";
import {Sorting} from "../enums";


export const getAll = async (page:number = 1, query?:string,sorting:Sorting = Sorting.CREATED ,limit:number = 10):Promise<NoteResponseAll> => {
    ...
    const {data} = await http.get<NoteResponseAll>(`${routes.all}?${params.toString()}`)
    return data;
}
...
export const updateById = async (id:number,note:NoteUpdate):Promise<Note> => {
