...
const NoteForm = ({onClose, note,tags,variant}: NoteFormProps) => {
    ...
    const handleSubmit = (values:InitialValues,helpers:FormikHelpers<InitialValues>) => {
       switch (variant) {
           ...
           case ModalVariant.UPDATE:
               ...
               updateNote.mutate({id:note.id,data:values},{
                   onSuccess: () => {
                       helpers.resetForm()
                       onClose()
                   }
    ...
    const schema = Yup.object({
        title: Yup.string().min(3,"Title must be at least 2 characters").max(50,'Title must be max 50 characters').required("Title is required"),
        content: Yup.string().max(500,'Content must be max 500 characters').required("Content is required"),
        tag: Yup.string().oneOf(tags,'Please choose a valid tag').required('Tag is required')
    })
